            ...
            while len(queue) > 0:  # dfs
                ...
                for nr, nc, nm in [
                    ...
                    if keypad[nr][nc] == "-1":
                        continue
                    if keypad[nr][nc] == y:
                        if min_press < len(moves) + 1:
                            break
                        min_press = len(moves) + 1
                        possibility_list.append(moves + nm + "A")
                    else:
                        queue = [((nr, nc), moves + nm)] + queue
                else:
                    continue
                break
            sequences[(x, y)] = possibility_list
    return sequences


def apply(string, seqs):
...
direction_keypad = [["-1", "^", "A"], ["<", "v", ">"]]

dir_seqs = compute_seqs(direction_keypad)
