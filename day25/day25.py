...
def part1():
    with open ('input') as f:
        ...
        for block in blueprints:
            print("======")
            matrix= []
            for row in block.splitlines():
                matrix.append(list(map(int, list((row.replace("#", "1").replace(".", "0"))))))
            matrix_len = [sum(i) - 1 for i in list(zip(*matrix))]
            print(matrix_len)
            if matrix[0][0] == 1:
                locks.append(matrix_len)
            else:
                keys.append(matrix_len)
