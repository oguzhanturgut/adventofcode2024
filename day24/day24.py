...
def part1():
    with open('input') as f:
        wires, simulations = f.read().split('\n\n')
        wires = {wire.split(': ')[0]: int(wire.split(': ')[1]) for wire in wires.split('\n')}
        simulations = [sim for sim in simulations.split('\n')]
        while len(simulations) > 0:
            for sim in simulations:
                left, operator, right, output = sim.replace(' -> ', ' ').split(' ')
                # print(inputs, output)
        ...
        results = tuple(sorted([(wire, value) for wire, value in wires.items() if wire.startswith('z')], reverse=True))
        print(int("".join([str(result[1]) for result in results]), 2))
