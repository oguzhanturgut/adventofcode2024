    ...
    with open("input.txt") as f:
        ...
        for secret in data:
            ...
            for _ in range(2000):
                ...
                track = set()
            for x in range(len(buyer) - 4):
                sub = buyer[x : x + 5]
                seq = (sub[1] - sub[0], sub[2] - sub[1], sub[3] - sub[2], sub[4] - sub[3])
                if seq in track:
                    continue
                track.add(seq)
                total_of_seqs[seq] += sub[4]
        print(max(total_of_seqs.values()))
