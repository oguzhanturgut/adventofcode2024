...
def part1():
  ...
  def is_possible(design):
    if len(design) == 0:
      return 1
    for length in range(min(len(design),max_pattern_size) + 1):
      if design[:length] in patterns and is_possible(design[length:]):
        return 1
    return 0
  ...
  def is_possible(design):
    ...
    for length in range(min(len(design),max_pattern_size) + 1):
      if design[:length] in patterns:
        count += is_possible(design[length:])
    return count
